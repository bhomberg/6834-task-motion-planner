...
When the pose generator is called by the interface layer, it generates a set of random poses for each action. Each pose is made up of a set of waypoints. For example for the \textit{pickup} action, it generates the following waypoints: near the cylinder and pointing towards it, touching the cylinder, lifting the cylinder, and a standard home position. It passes one pose to the interface layer at a time, if none of the poses are feasible, the interface layer backtracks resets the pose generator. The TryRefine method of the interface layer attempts to generate a feasible motion plan linking a set of waypoints.
...
The most important limitation of this implementation is to remember that it is an interface layer.  
It does not include a task planner and a motion planner: you must set those up yourself as ROS services and make sure that the output and input message types specify those outlined in the API.  
Depending on what planners you use, setting them up and installing them may be nontrivial.  
Different planners are effective for different tasks and some planners may require extensive installation.  
When you pick your task and motion planners, remember that this interface layer is set up to only work with task planners that use the PDDL specification language.  
If you choose to use another specification system, you will need to translate between PDDL and your system.

On a more detailed level, it is important to note that this planner is not guaranteed to find an optimal solution, or even to find a solution at all.  
